...
10C itself was audit/planning only — no migrations, code, or Realtime channels were added in that pass. **Phase 10D has since implemented the §3/§9 RLS fix** (`20260724000000_pass31_round_content_participant_rls.sql` + `backend/tests/test_pass31_round_content_rls.py`). **Phase 10E has since implemented Option C** (Broadcast, backend-mediated, notify-then-refetch — see the "Recommended path" section below for the full status and `docs/REALTIME_CROSSFIRE_PHASE10_PLAN.md` for the implementation writeup). See `docs/ROUND_SIMULATION_PERMISSIONS.md`'s "Update — Phase 9 / Phase 10D" section for the current, accurate policy summary.

**Separate follow-on finding (Pass 32/33, not a §3 read-policy gap):** live-testing the §3 fix surfaced a *different* bug class in two of the tables this doc discusses — `round_coach_annotations` (row 33 below) and `round_finding_ratings` (row 34 below) each had a `FOR ALL USING (...)` write policy with no explicit `WITH CHECK`, and in both cases the `USING` clause was built on a self-declarable column (`coach_id`/`rater_id`) rather than a verified round relationship — so Postgres reusing `USING` as the write-time check let any authenticated user forge a row by setting that column to their own `auth.uid()`, regardless of any real relationship to the round. Both were fixed with an explicit `WITH CHECK` mirroring `_require_coach_or_owner_access` (owner or joined `role='coach'`); reads were left untouched. This is orthogonal to the read-policy gap §3 describes — it's a write-authorization bug, not an under-delivery-on-read issue — but is noted here since it was found while validating this doc's own recommended fix. See `TestWriteCheckSweep` in `backend/tests/test_pass31_round_content_rls.py` for the full audit and regression guard.
...
3. ~~**Phase 10E**~~ **Done**: Option C (Broadcast, backend-mediated, notify-then-refetch) — not Option B, given its higher ongoing drift/operational risk for no clear benefit at Dissio's current scale. Implemented as designed here: backend is the sole writer (Realtime's REST broadcast endpoint, not a held-open channel), payload carries no round content, `room_id` (never `invite_code`) is the channel name, and a new `realtime.messages` RLS policy (Pass 34) adds real Realtime Authorization on top of the room-UUID-as-channel-name approach this doc originally sanctioned. 10B's polling remains the fallback/transport-of-record, unchanged. Full writeup: `docs/REALTIME_CROSSFIRE_PHASE10_PLAN.md`'s "Phase 10E — Broadcast notify-then-refetch" section.
